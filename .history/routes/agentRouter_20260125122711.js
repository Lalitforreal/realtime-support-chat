...
router.get('/dashboard',requireAgent,async function(req,res){
    ...
    const closedtickets = await ticketModel.find({status : "closed"}).sort({updatedAt : -1})
    .limit(5);//important

    res.render("agent-dashboard",{ tickets ,closedtickets});
});
...
router.get('/ticket/:ticketId',requireAgent,async function(req,res){
    ...
    .sort({createdAt : 1}).limit(3); //without .populate(tickedId) just messga with populara the whole object as a ref, now we have boht msg+ticket data
